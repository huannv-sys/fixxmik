...
const NetworkTrafficAdvanced: React.FC<NetworkTrafficAdvancedProps> = ({
  ...
  const { connectionStats, refetch: refetchConnectionStats } =
  ...
  const handleRefresh = () => {
    ...
    if (activeTab === "ports" && refetchConnectionStats) {
      ...
      axios
        .post(`/api/devices/${deviceId}/clear-cache/connection-stats`)
        .then(() => {
      ...
      .map((metric, index) => {
        ...
        if (index > 0 && timeFrameData[index - 1]) {
          ...
          if (timeDiff > 0) {
            ...
            console.log(
              `Mẫu ${index}: DLbw=${downloadBandwidth}, ULbw=${uploadBandwidth}, ` +
      ...
      if (activeTab === "ports" && refetchConnectionStats) {
        axios
          ...
          .then(() => {
            refetchConnectionStats();
            console.log(
              "Đã xóa cache và tải lại connection stats cho tab Ports",
            );
          })
          .catch((error) => {
              ...
              connectionStats.top10Ports.length > 0 ? (
                      ...
                      {connectionStats.top10Ports.map((portInfo, index) => (
                        ...
                        >
                          ...
                          <td className="py-2 px-3 text-blue-400 font-mono">
                            {portInfo.connectionCount}
                          </td>
                          <td className="py-2 px-3 text-green-400 font-mono">
                            {portInfo.percentage.toFixed(1)}%
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="flex items-center justify-center h-40">
                  <p className="text-gray-400">
                    Không có dữ liệu về cổng kết nối
                  </p>
                </div>
              )}
            </CardContent>
                ...
                connectionStats.top10Destinations.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full bg-transparent text-sm">
                      ...
                      <tbody>
                        {connectionStats.top10Destinations.map(
                          (dest, index) => (
                            <tr
                              key={`dest-${index}`}
                              className="border-b border-gray-800"
            ...
            <div className="flex items-center space-x-2">
              <div className="text-xs text-gray-400">
                ...
                <span className="text-gray-300">
                  {getCurrentTrafficStats().lastUpdate}
                </span>
              </div>
            </div>
              ...
              <CardContent className="p-4 pt-0">
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-slate-900 rounded-md p-3">
                    ...
                    <div className="text-blue-400 font-mono font-medium text-lg">
                      {formatBandwidth(
                        (currentStats.upload + currentStats.download) * 125000,
                        2,
                      )}
                    </div>
                  </div>
