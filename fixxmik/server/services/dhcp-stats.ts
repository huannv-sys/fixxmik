...
class DHCPStatsService {
  private cachedStats = new Map<
    number,
    { stats: DHCPStats; timestamp: number }
  >();
  ...
  async getDHCPStats(deviceId: number): Promise<DHCPStats | null> {
    try {
      ...
      if (!connected) {
        logger.warn(
          `Không thể kết nối đến thiết bị ID ${deviceId} để lấy thông tin DHCP`,
        );
        return null;
      }
      ...
      if (
        !dhcpServers ||
        !Array.isArray(dhcpServers) ||
        dhcpServers.length === 0
      ) {
        logger.warn(
          `Không tìm thấy DHCP server nào trên thiết bị ID ${deviceId}`,
        );
        return null;
      }
      ...
      if (!dhcpPools || !Array.isArray(dhcpPools) || dhcpPools.length === 0) {
        logger.warn(
          `Không tìm thấy DHCP pool nào trên thiết bị ID ${deviceId}`,
        );
        return null;
      }

      // Lấy thông tin về DHCP leases
      const dhcpLeases = await client.executeCommand(
        "/ip/dhcp-server/lease/print",
      );
      const activeLeases = Array.isArray(dhcpLeases)
        ? dhcpLeases.filter(
            (lease) => lease.status === "bound" || lease.status === "waiting",
          )
        : [];
      ...
      const poolRanges = dhcpPools.map((pool: any) => {
        ...
        const rangeMatches = (ranges || "").match(
          /(\d+\.\d+\.\d+\.\d+)-(\d+\.\d+\.\d+\.\d+)/,
        );
        let start = "";
        let end = "";
      ...
      logger.error(
        `Lỗi khi lấy thông tin DHCP stats cho thiết bị ID ${deviceId}:`,
        error,
      );
      return null;
    }
