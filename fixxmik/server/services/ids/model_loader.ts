...
interface TrafficMemory {
  ...
  dosAttackData: Map<
    string,
    { packets: number; bytes: number; timestamps: Date[] }
  >;
  bruteforceData: Map<
    string,
    Map<number, { count: number; timestamps: Date[] }>
  >;
...
class ModelLoader {
  ...
  private cleanOldEntries(): void {
    ...
    this.trafficMemory.portScanTimestamps.forEach((timestamps, key) => {
      const filteredTimestamps = timestamps.filter(
        (ts) =>
          now.getTime() - ts.getTime() < THRESHOLDS.PORT_SCAN.TIME_WINDOW_MS,
      );
    ...
    this.trafficMemory.dosAttackData.forEach((data, key) => {
      const filteredTimestamps = data.timestamps.filter(
        (ts) =>
          now.getTime() - ts.getTime() < THRESHOLDS.DOS_ATTACK.TIME_WINDOW_MS,
      );
    ...
    this.trafficMemory.bruteforceData.forEach((portMap, sourceIp) => {
      ...
      portMap.forEach((data, port) => {
        const filteredTimestamps = data.timestamps.filter(
          (ts) =>
            now.getTime() - ts.getTime() < THRESHOLDS.BRUTEFORCE.TIME_WINDOW_MS,
        );
  ...
  private detectPortScan(
    ...
    const portRatio = Math.min(
      1,
      uniquePorts / THRESHOLDS.PORT_SCAN.UNIQUE_PORTS,
    );
  ...
  private detectDosAttack(
    ...
    const packetRatioProb = Math.min(
      1,
      packetRate / THRESHOLDS.DOS_ATTACK.PACKET_RATE,
    );
    const byteRatioProb = Math.min(
      1,
      byteRate / THRESHOLDS.DOS_ATTACK.FLOW_BYTES_RATE,
    );
  ...
  private detectBruteforce(
    ...
    const isCommonPort =
      THRESHOLDS.BRUTEFORCE.COMMON_PORTS.includes(destinationPort);
    if (!isCommonPort) {
      return { isAnomaly: false, probability: 0 };
    }
  ...
  public async predict(
    ...
    try {
      ...
      const portScanResult = this.detectPortScan(
        sourceIp,
        destinationIp,
        destinationPort,
      );
      const dosResult = this.detectDosAttack(
        sourceIp,
        destinationIp,
        totalBytes,
        totalPackets,
      );
      const bruteforceResult = this.detectBruteforce(
        sourceIp,
        destinationIp,
        destinationPort,
      );
      ...
      if (highestResult.isAnomaly) {
        switch (highestResult.type) {
          case "PORT_SCAN":
            description =
              "Port scanning activity detected - multiple ports accessed within a short time window";
            break;
          case "DOS_ATTACK":
            description =
              "Possible DoS attack - high rate of packets directed at the target";
            break;
          case "BRUTEFORCE":
            description =
              "Possible brute force attack - repeated connection attempts to critical service";
            break;
        }
      }
