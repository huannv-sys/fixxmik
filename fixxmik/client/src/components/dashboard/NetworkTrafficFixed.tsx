...
import {
  ...
  formatBitrate,
  formatBandwidth,
  formatAccumulatedBytes,
} from "@/hooks/use-data-formatter";
...
export default function NetworkTrafficFixed({
  deviceId,
}: NetworkTrafficFixedProps) {
  ...
  const formatTrafficData = () => {
    ...
    timeFrameData = timeFrameData.filter(
      (item) => new Date(item.timestamp).getTime() > pastTime,
    );
    ...
    const formattedData = timeFrameData.map((item) => {
      ...
      const download =
        typeof item.downloadBandwidth === "number" ? item.downloadBandwidth : 0;
      const upload =
        typeof item.uploadBandwidth === "number" ? item.uploadBandwidth : 0;
      ...
      return (
        <div className="p-4">
          <Card className="border-none bg-slate-950">
            <CardHeader className="p-4 pb-2">
              <CardTitle className="text-sm font-medium">
                Nguồn lưu lượng phổ biến
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-0">
              {connectionStats?.top10Sources &&
              connectionStats.top10Sources.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="min-w-full bg-transparent text-sm">
                    <thead>
                      <tr className="border-b border-gray-800">
                ...
                <div className="text-center py-4">
                  <p className="text-gray-400">
                    Không có dữ liệu về nguồn lưu lượng
                  </p>
                </div>
              )}
            </CardContent>
      ...
      return (
        <div className="p-4">
          <Card className="border-none bg-slate-950">
            <CardHeader className="p-4 pb-2">
              <CardTitle className="text-sm font-medium">
                Đích đến lưu lượng phổ biến
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-0">
              {connectionStats?.top10Destinations &&
              connectionStats.top10Destinations.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="min-w-full bg-transparent text-sm">
                    <thead>
                      <tr className="border-b border-gray-800">
                ...
                <div className="text-center py-4">
                  <p className="text-gray-400">
                    Không có dữ liệu về đích đến lưu lượng
                  </p>
                </div>
              )}
            </CardContent>
                  ...
                  <div className="col-span-4 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="bg-slate-900 rounded-lg p-3">
                      ...
                      <div className="text-xs text-gray-500 mt-2 flex justify-between">
                        <span>
                          Gói tin: {formatPacketRate(stats.rxPacketRate)}
                        </span>
                    ...
                    <div className="bg-slate-900 rounded-lg p-3">
                      ...
                      <div className="text-xs text-gray-500 mt-2 flex justify-between">
                        <span>
                          Gói tin: {formatPacketRate(stats.txPacketRate)}
                        </span>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        ticks={getYAxisTicks()}
                        tickFormatter={(value) =>
                          formatBandwidth(Number(value) * 125000, 1)
                        }
                        tick={{ fontSize: 10 }}
                        stroke="#666"
                      />
                      <Tooltip
                        formatter={(value) => [
                          formatBandwidth(Number(value) * 125000, 2),
                          "",
                        ]}
                        contentStyle={{
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-slate-700/70 rounded p-1.5 text-xs">
                  ...
                  <span className="text-white font-mono">
                    {dhcpStats?.activeLeases
                      ? `${dhcpStats.activeLeases} / ${dhcpStats.poolSize}`
                      : "----"}
                  </span>
                </div>
                <div className="bg-slate-700/70 rounded p-1.5 text-xs">
                  ...
                  <span className="text-white font-mono">
                    {dhcpStats?.usagePercentage
                      ? `${Math.round(dhcpStats.usagePercentage)}%`
                      : "---"}
                  </span>
                </div>
              </div>
    ...
    <div className="bg-slate-950 p-4 rounded-xl shadow-lg w-full">
      ...
      >
        <TabsList className="w-full bg-slate-800 p-1 h-auto flex flex-wrap md:flex-nowrap">
          <TabsTrigger
            value="traffic"
            className="flex-1 text-xs md:text-sm py-1.5 data-[state=active]:bg-slate-700"
          >
            Lưu lượng
          </TabsTrigger>
          <TabsTrigger
