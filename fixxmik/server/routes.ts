...
import {
  getDeviceArpTable,
  convertArpEntriesToNetworkDevices,
} from "./services/arp-api";
import { db } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      if (req.query.check === "true") {
        ...
        for (const device of devices) {
          if (device.ipAddress) {
            ...
            const isOnline = await mikrotikService.checkDeviceOnline(
              device.ipAddress,
            );
  ...
  router.get("/devices/:id/logs", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(
        `[LOGS API] Device found: ${device.name} (${device.ipAddress})`,
      );
      ...
      console.log(
        `[LOGS API] Successfully retrieved ${result.data?.length || 0} logs`,
      );
      res.json(result);
    } catch (error) {
  ...
  router.get("/devices/:id/capsman", async (req: Request, res: Response) => {
    try {
      ...
      if (capsmanAPs.length === 0) {
        console.log(
          `Không có AP nào, bắt đầu thu thập dữ liệu CAPsMAN cho thiết bị ${deviceId}...`,
        );
        try {
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Đã làm mới thông tin và tìm thấy ${capsmanAPs.length} CAPsMan APs`,
        );
    ...
    try {
      ...
      try {
        ...
        if (arpEntries && arpEntries.length > 0) {
          ...
          const networkDevices = convertArpEntriesToNetworkDevices(
            arpEntries,
            deviceId,
          );

          console.log(
            `Đã lấy ${networkDevices.length} ARP entries từ thiết bị ${deviceId}`,
          );
        ...
        console.error(
          `Lỗi khi lấy ARP entries từ thiết bị ${deviceId}:`,
          arpError,
        );
      }
      ...
      if (device.hasCAPsMAN) {
        ...
        const clients =
          await capsmanService.getCapsmanClientsByDevice(deviceId);
      ...
      console.error(
        "Lỗi khi lấy danh sách clients/ARP entries theo thiết bị:",
        error,
      );
  ...
  router.post("/clients/scan", async (req: Request, res: Response) => {
    try {
      ...
      if (devices.length > 0) {
        ...
        const directDevices =
          await mikrotikService.getNetworkNeighbors(routerId);
        console.log(
      ...
      try {
        ...
        if (
          ...
          return res.status(400).json({
            success: false,
            message:
              "Dữ liệu không hợp lệ. Cần các trường: sourceIp, destinationIp, sourcePort, destinationPort, protocol, bytes, packetCount, flowDuration, deviceId",
          });
        }
    ...
    try {
      ...
      if (device && device.length > 0) {
        try {
          ...
          if (firewallLogs && firewallLogs.length > 0) {
            console.log(
              `Đã tìm thấy ${firewallLogs.length} log firewall từ thiết bị ${device[0].name}`,
            );

            // Chuyển đổi dữ liệu firewall log thành định dạng của anomalies
            const realFirewallAnomalies = firewallLogs.map(
              (log: any, index: number) => {
                ...
                if (message.includes("SYN flood")) {
                  ...
                  message.includes("port scan") ||
                  (protocol === "tcp" && message.includes("SYN"))
                ) {
                ...
                return {
                  ...
                  timestamp: new Date(
                    new Date().setMinutes(new Date().getMinutes() - index * 5),
          ...
          console.warn(
            "Không thể lấy dữ liệu từ thiết bị Mikrotik:",
            mikrotikError,
          );
      ...
      if (anomalies.length === 0) {
        ...
        const device = await db
          ...
          .limit(1);
        const deviceIp =
          device && device.length > 0 ? device[0].ipAddress : "192.168.1.1";
        ...
        anomalies = [
          {
            ...
            details: {
              message:
                "Phát hiện nhiều kết nối thất bại đến SSH từ 203.113.131.45",
          ...
          },
          {
            ...
            details: {
              message:
                "Phát hiện quét cổng từ 121.45.67.89 đến nhiều cổng dịch vụ",
              sourceIp: "121.45.67.89",
              destinationIp: deviceIp,
            },
          },
          {
            ...
            details: {
              message:
                "Phát hiện nhiều kết nối đồng thời từ 45.76.123.45 đến cổng HTTPS",
              sourceIp: "45.76.123.45",
              destinationIp: deviceIp,
            },
          },
      ...
      try {
        ...
        if (!["port_scan", "dos_attack", "bruteforce"].includes(type)) {
          return res.status(400).json({
            success: false,
            message:
              "Loại tấn công không hợp lệ. Hỗ trợ: port_scan, dos_attack, bruteforce",
          });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          const connectionData = await client.executeCommand(
            "/ip/firewall/connection/print",
          );

          console.log(
            `Đã nhận ${connectionData.length} kết nối từ thiết bị ${device.name}`,
          );
          ...
          console.log(
            `Đã tạo ${trafficEntries.length} mục dữ liệu lưu lượng cho phân tích`,
          );
            ...
            data: {
              ...
              anomalyPercentage:
                trafficEntries.length > 0
                  ? (anomalies.length / trafficEntries.length) * 100
                  : 0,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          console.log(
            `Đã tìm thấy ${filterRules.length} firewall filter rules từ thiết bị ${device.name}`,
          );

          // Lấy thêm thông tin NAT rules
          const natRules = await client.executeCommand(
            "/ip/firewall/nat/print",
          );

          console.log(
            `Đã tìm thấy ${natRules.length} firewall NAT rules từ thiết bị ${device.name}`,
          );
          ...
          console.log(
            `Đã tìm thấy ${addressLists.length} address lists từ thiết bị ${device.name}`,
          );
          ...
          console.log(
            `Số lượng filter rules đã định dạng: ${formattedFilterRules.length}`,
          );
  ...
  router.post(
    "/devices/:id/firewall/filter/:ruleId/toggle",
    async (req: Request, res: Response) => {
      try {
        const deviceId = parseInt(req.params.id);
        ...
        try {
          ...
          const ruleInfo = await client.executeCommand(
            "/ip/firewall/filter/print",
            [`?.id=${ruleId}`],
          );
  ...
  // API endpoint để bật/tắt firewall rule
  router.post(
    "/devices/:deviceId/firewall/filter/:ruleId/toggle",
    async (req: Request, res: Response) => {
      try {
        const deviceId = parseInt(req.params.deviceId);
        const ruleId = req.params.ruleId;
        ...
        try {
          ...
          const currentRule = await client.executeCommand(
            "/ip/firewall/filter/print",
            [`?.id=${ruleId}`],
          );
          ...
          const updatedRule = await client.executeCommand(
            "/ip/firewall/filter/print",
            [`?.id=${ruleId}`],
          );
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Đang lấy logs cho thiết bị ${device.name} (${device.ipAddress})...`,
        );
        ...
        console.log(
          `Đã tìm thấy ${formattedLogs.length} system logs từ thiết bị ${device.name}`,
        );
  ...
  function getSeverityFromTopics(
    topics: string,
        ...
        try {
          ...
          for (let i = 0; i < timePoints; i++) {
            const timestamp = new Date(
              now.getTime() - (timePoints - i) * 15 * 60 * 1000,
            ); // 15 phút một lần
            const download =
              (totalRxBytes / timePoints) * (0.8 + Math.random() * 0.4); // Thêm độ lệch
            const upload =
              (totalTxBytes / timePoints) * (0.8 + Math.random() * 0.4);
          ...
          res.json({
            ...
            data: sampleData,
            message:
              "Dữ liệu băng thông đang được tạo từ thông tin thực của thiết bị",
          });
          return;
        } catch (error) {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          // Tính tổng lưu lượng
          const totalTraffic = interfaceStats.reduce(
            (sum, iface) => sum + iface.totalBytes,
            0,
          );
          ...
          interfaceStats.forEach((iface) => {
            iface.percentage =
              totalTraffic > 0 ? (iface.totalBytes / totalTraffic) * 100 : 0;
          });
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Import service theo yêu cầu để tránh circular dependency
        const { connectionStatsService } = await import(
          "./services/connection-stats"
        );

        // Lấy thông tin connection stats
        const connectionStats =
          await connectionStatsService.getConnectionStats(deviceId);

        if (!connectionStats) {
          return res.status(404).json({
            success: false,
            message:
              "Không thể lấy thông tin connection tracking từ thiết bị này",
          });
        }
        ...
        if (
          ...
          connectionStats.top10Ports.length === 0
        ) {
          logger.warn(
            "Không tìm thấy dữ liệu ports từ Mikrotik. Sử dụng dữ liệu mẫu cho API.",
          );
          connectionStats.top10Ports = [
            {
              port: 80,
        ...
        logger.info(
          `Trả về connection stats với ${connectionStats.top10Ports.length} ports`,
        );
  ...
  router.post(
    "/devices/:id/clear-cache/:service",
    async (req: Request, res: Response) => {
      try {
        const deviceId = parseInt(req.params.id);
